...
class ReportService:
    ...
    def get_latest_report(self) -> Optional[Dict[str, Any]]:
        ...
        # Look for Phase 5 generated reports
        report_files = [
            'phase5-security-report.json',
        ...
        for filename in dashboard_files:
            ...
            if dashboard_path.exists():
                try:
                    ...
                    logger.error(f"Error loading dashboard data {filename}: {e}")
    ...
    def get_findings(
        ...
        if severity:
            filtered = [f for f in filtered if f.get('severity') == severity.upper()]
        
        if owasp_category:
            filtered = [f for f in filtered if f.get('owasp_category') == owasp_category]
    ...
    def get_finding_by_id(self, finding_id: str) -> Optional[Dict[str, Any]]:
        ...
        for finding in all_findings:
            if finding.get('id') == finding_id or finding.get('cve_id') == finding_id:
                ...
                finding['fix_suggestion'] = self._get_fix_suggestion(finding_id)
                return finding
    ...
    def _get_fix_suggestion(self, finding_id: str) -> Optional[Dict[str, Any]]:
        ...
        fix_files = [
            'phase5-fix-suggestions.json',
            'fix-suggestions.json'
        ]
        
        for filename in fix_files:
            fix_path = Path(filename)
            if fix_path.exists():
                try:
                    ...
                    logger.error(f"Error loading fixes {filename}: {e}")
