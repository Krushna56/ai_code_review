...
class JavaParser:
    ...
    def __init__(self):
        if not TREE_SITTER_AVAILABLE:
            raise ImportError("tree-sitter-languages required. Install with: pip install tree-sitter-languages")
    ...
    def parse_source(self, source_code: str, file_path: str = None) -> List[Dict[str, Any]]:
        ...
        for node in root_node.children:
            if node.type in ['class_declaration', 'interface_declaration', 'enum_declaration']:
                class_chunks = self._extract_class(node, source_code, source_lines, file_path, package_name)
                chunks.extend(class_chunks)
            elif node.type == 'method_declaration':
                # Top-level methods (rare in Java but possible)
                method_chunk = self._extract_method(node, source_code, source_lines, file_path)
                if method_chunk:
    ...
    def _extract_class(self, class_node, source_code: str, source_lines: List[str], 
        ...
        if class_body:
            ...
            class_signature = self._get_class_signature(class_node, source_code)
            class_chunk = {
                'type': 'class',
                'name': class_name,
                'file': file_path,
    ...
    def _get_class_signature(self, class_node, source_code: str) -> str:
        ...
        return source_code[class_node.start_byte:class_node.end_byte][:200]  # Fallback
