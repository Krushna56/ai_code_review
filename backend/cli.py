...
def cmd_cve_check(args):
    ...
    if dependencies:
        ...
        if all_vulns:
            print(f"\nTop Vulnerabilities:")
            sorted_vulns = sorted(all_vulns, key=lambda v: severity_counts.get(
                v['severity'], 0), reverse=True)
...
Examples:
  ...
  python cli.py stats
    ...
    index_parser.add_argument(
        '--clear', action='store_true', help='Clear existing index first')
    ...
    query_parser = subparsers.add_parser(
        'query', help='Ask a security question')
    query_parser.add_argument(
        'question', help='Natural language security question')
    query_parser.add_argument(
        '--max-results', type=int, default=5, help='Max code chunks to retrieve')
    query_parser.add_argument(
        '--show-code', action='store_true', help='Show code previews in output')
    ...
    secrets_parser = subparsers.add_parser(
        'scan-secrets', help='Scan for hardcoded secrets')
    ...
    cve_parser = subparsers.add_parser(
        'cve-check', help='Check dependencies for CVEs')
    ...
    cve_parser.add_argument(
        '--cache-dir', help='Directory for caching CVE results')
