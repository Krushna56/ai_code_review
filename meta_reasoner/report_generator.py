...
class ReportGenerator:
    ...
    def generate_markdown_report(
        ...
        md.append(f"- **Critical Issues:** {sum(1 for i in issues if i.get('severity') in ['critical', 'high'])}")
        md.append(f"- **Security Issues:** {summary.get('security_issues', 0)}\n")
        ...
        for i, issue in enumerate(top_issues, 1):
            ...
            if 'also_found_by' in issue:
                md.append(f"*Also detected by: {', '.join(issue['also_found_by'])}*\n")
