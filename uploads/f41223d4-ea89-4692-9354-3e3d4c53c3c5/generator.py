...
class AgentGenerator:
    ...
    def set_provider(self, provider: str):
        ...
        if provider.lower() not in ["openai", "watsonx"]:
            raise ValueError(
                f"Unsupported provider: {provider}. Use 'openai' or 'watsonx'.")
    ...
    def _initialize_model(self):
        ...
        if self.provider == "openai":
            ...
            if not api_key and st is not None:
                st.warning(
                    "OpenAI API Key not found in environment. Please enter it below.")
                api_key = st.text_input(
                    "Enter OpenAI API Key:", type="password", key="openai_key_input")
                if api_key:
            ...
            if (not api_key or not project_id) and st is not None:
                st.warning(
                    "WatsonX credentials not found in environment. Please enter them below.")
                col1, col2 = st.columns(2)
                with col1:
                    api_key = st.text_input(
                        "Enter WatsonX API Key:", type="password", key="watsonx_key_input")
                with col2:
                    project_id = st.text_input(
                        "Enter WatsonX Project ID:", key="watsonx_project_input")
