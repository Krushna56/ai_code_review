...
class CodeAnalyzer:
        ...
        try:
            ...
            if config.ENABLE_LLM_AGENTS:
                ...
                if len(original_code) > 10000:
                    logger.info(f"Large file detected ({len(original_code)} chars), analyzing key sections")
    ...
    def _run_llm_analysis(self, code: str, metrics: Dict, static_issues: List) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"LLM security analysis complete: {len(security_result.get('issues', []))} issues found")
            ...
            logger.info(f"LLM refactoring complete: {len(refactor_result.get('suggestions', []))} suggestions")
    ...
    def _run_llm_analysis_chunked(self, code: str, metrics: Dict, static_issues: List, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            critical_sections = self._extract_critical_sections(code, metrics, static_issues)
            ...
            for section in critical_sections[:5]:  # Limit to top 5 sections
                ...
                security_result = self.security_agent.analyze(section['code'], context)
                if 'issues' in security_result:
                    all_security_issues.extend(security_result['issues'])
                
                refactor_result = self.refactor_agent.analyze(section['code'], context)
                if 'suggestions' in refactor_result:
                    all_refactor_suggestions.extend(refactor_result['suggestions'])
            
            insights['security'] = {'issues': all_security_issues, 'analyzed_sections': len(critical_sections)}
            insights['refactoring'] = {'suggestions': all_refactor_suggestions, 'analyzed_sections': len(critical_sections)}
    ...
    def _extract_critical_sections(self, code: str, metrics: Dict, static_issues: List) -> List[Dict[str, Any]]:
        ...
        sections.sort(key=lambda s: (not s.get('has_issues', False), -len(s['code'])))
...
def analyze_codebase(input_path, output_path):
    ...
    if CVE_DETECTION_AVAILABLE:
        try:
                ...
                for file in files:
                    if file.endswith(('.py', '.java', '.js', '.go', '.rb', '.php', '.env', '.yml', '.yaml', '.json')):
                        ...
                        try:
                            with open(file_path, 'r', encoding='utf-8') as f:
                                code = f.read()
                            secrets = secret_detector.scan_code(code, file_path)
            ...
            if security_findings:
                logger.info(f"Found {len(security_findings)} hardcoded secrets")
        except Exception as e:
    ...
    if SECURITY_REPORTING_AVAILABLE and getattr(config, 'ENABLE_SECURITY_REPORTING', True):
        try:
            logger.info("Phase 5: Generating security reports and recommendations...")
            ...
            # Save JSON report
            json_report_path = os.path.join(output_path, "security_report.json")
            report_generator.save_report(security_report_data, json_report_path, format='json')
            ...
            report_generator.save_report(security_report_data, md_report_path, format='markdown')
            ...
            for finding in security_findings[:5]:  # Top 5 findings
                try:
                    fix = fix_generator.generate_fix_suggestion(finding, context={'language': 'python'})
            ...
            if fix_suggestions:
                fix_output_path = os.path.join(output_path, "fix_suggestions.json")
            ...
            dashboard_output_path = os.path.join(output_path, "dashboard_data.json")
